...
namespace UnityEngine
{
    public static class Utilities
    {
        ...
        public static void SetFieldByName(object o, string fieldName, object newValue)
        {
            ...
            if (f.FieldType == typeof(List<Component>))
                newValue = ConvertList<Component>(newValue);
            if (f.FieldType == typeof(List<GameObject>))
                newValue = ConvertList<GameObject>(newValue);
            if (f.FieldType == typeof(List<Transform>))
                newValue = ConvertList<Transform>(newValue);
        ...
        private static object ConvertList<T>(object list)
        {
            ...
            return iList.Select(ConvertElement).ToList();
        }
